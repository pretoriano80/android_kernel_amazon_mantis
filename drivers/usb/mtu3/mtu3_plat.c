...
static int ssusb_clks_enable(struct ssusb_mtk *ssusb)
{
	...
	if (ret) {
		dev_err(ssusb->dev, "failed to enable sys_clk since %i\n", ret);
		goto sys_clk_err;
	}

	ret = clk_prepare_enable(ssusb->free_clk);
	if (ret) {
		dev_err(ssusb->dev, "failed to enable free_clk since %i\n", ret);
		goto free_clk_err;
	}

	ret = clk_prepare_enable(ssusb->ahb_clk);
	if (ret) {
		dev_err(ssusb->dev, "failed to enable ahb_clk since %i\n", ret);
		goto ahb_clk_err;
	}

	ret = clk_prepare_enable(ssusb->dma_clk);
	if (ret) {
		dev_err(ssusb->dev, "failed to enable dma_clk since %i\n", ret);
		goto dma_clk_err;
	}
...
enum usb_dr_mode usb_get_default_mode(struct device *dev)
{
	...
	if (!strcmp(default_mode, "peripheral"))
		return USB_DR_MODE_PERIPHERAL;
	else if (!strcmp(default_mode, "host"))
...
static int ssusb_rscs_init(struct ssusb_mtk *ssusb)
{
	...
	if (ret) {
		dev_err(ssusb->dev, "failed to enable ssusb clks since %i\n", ret);
		goto clk_err;
	}
...
static int get_ssusb_rscs(struct platform_device *pdev, struct ssusb_mtk *ssusb)
{
	...
	ssusb->free_clk = devm_clk_get(dev, "free_ck");
	...
	ssusb->ahb_clk = devm_clk_get(dev, "ahb_ck");
	...
	for (i = 0; i < ssusb->num_phys; i++) {
		...
		if (ssusb->op_phys[i].phy == ERR_PTR(-ENODEV))
			...
			dev_err(dev, "get phy-%d failed since ret:%i\n", i, ret);
			return ret;
		}
	...
	if (of_property_read_bool(node, "extcon")) {
		...
		if (otg_sx->edev == ERR_PTR(-EPROBE_DEFER)) {
			...
			dev_err(ssusb->dev, "couldn't get extcon device since %i\n", ret);
			return ret;
		}
...
static int mtu3_probe(struct platform_device *pdev)
{
	...
	case USB_DR_MODE_OTG:
		if (ssusb->num_phys && !ssusb->op_phys[0].phy_disable) {
			...
			dev_info(dev, "Port0 USB PHY had been required to disabled.\n");
	...
	dev_info(dev, "===mtu3_probe u2:u3 ports: %u:%u in ssusb 0x%p ===\n",
					ssusb->u2_ports, ssusb->u3_ports, ssusb);
